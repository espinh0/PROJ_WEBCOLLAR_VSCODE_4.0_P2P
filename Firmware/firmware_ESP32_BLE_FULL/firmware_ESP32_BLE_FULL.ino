...
static void uiSetDual(bool enabled) {
  portENTER_CRITICAL(&uiMux);
  uiDual = enabled;
  portEXIT_CRITICAL(&uiMux);
}
...
static void processCommandLine_core1(const char* lineC) {
  ...
  // DUALON/DUALOFF
  if (up == "DUALON" || up == "DUAL ON") {
    dualModeEnabled = true;
    enqueueTx("OK DUAL ON");
    uiSetDual(true);
    uiLogLine("DUAL ON");
    return;
  }
  if (up == "DUALOFF" || up == "DUAL OFF") {
    dualModeEnabled = false;
    enqueueTx("OK DUAL OFF");
    uiSetDual(false);
    uiLogLine("DUAL OFF");
    return;
  }
